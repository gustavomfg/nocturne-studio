...
export const changeControlDecisionSchema = changeControlChangeSchema.extend({ status: z.enum(['accepted', 'rejected']) }).strict()
export const changeControlHunkSchema = z.object({ conversationId: idSchema, hunkId: idSchema }).strict()
export const changeControlHunkEditSchema = changeControlHunkSchema.extend({ finalPatch: z.string().max(200_000) }).strict()
export const changeControlHunkDecisionSchema = changeControlHunkSchema.extend({ status: z.enum(['accepted', 'rejected']) }).strict()
...
export const rendererStatsSchema = z.object({
  ...
  renderCounts: z.object({
    ...
    agentActivity: z.number().int().nonnegative().max(1_000_000),
  }).strict(),
