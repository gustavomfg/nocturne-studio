# Índice da documentação

[English](README.md)

English é o idioma público canônico. Documentos voltados ao usuário têm um
companheiro `.pt-BR.md` com o mesmo escopo técnico. Comandos, caminhos, nomes de
API, avisos e afirmações de segurança são mantidos iguais nos dois arquivos.

## Documentação pública para usuários

- [Instalação](installation.pt-BR.md)
- [Primeiro uso](getting-started.pt-BR.md)
- [Providers e modelos](providers.pt-BR.md)
- [Configuração](configuration.pt-BR.md)
- [Integração Codex CLI](codex-integration.pt-BR.md)
- [Modos Review, Build e Docs](modes.pt-BR.md)
- [Segundo Cérebro](second-brain.pt-BR.md) e [Awareness](awareness.pt-BR.md)
- [Backup, restauração e recuperação](backup-and-recovery.pt-BR.md)
- [Atualizações](updates.pt-BR.md)
- [Fronteiras de segurança](security.pt-BR.md) e [privacidade](privacy.pt-BR.md)
- [Solução de problemas](troubleshooting.pt-BR.md)
- [Diagnóstico](diagnostics.pt-BR.md)
- [Compatibilidade](compatibility.pt-BR.md)

## Documentação para contribuidores

- [Arquitetura](architecture.pt-BR.md)
- [Desenvolvimento](development.pt-BR.md)
- [Detalhes do Docs Mode](docs-mode.pt-BR.md)
- [Recuperação do Build](build-recovery.pt-BR.md)
- [Contribuindo](../CONTRIBUTING.pt-BR.md)
- [Código de Conduta](../CODE_OF_CONDUCT.pt-BR.md)

## Documentação interna/de mantenedores

Prontidão da release, GitHub Actions, migrações do banco, contrato de Provider,
orçamentos de desempenho, identidade do produto, auditoria de segurança,
planos e notas históricas de release permanecem no repositório para rastreio de
engenharia. Não são promessas de suporte ao usuário e nem todos precisam de
tradução.

A [estratégia de modelos](model-strategy.pt-BR.md) é uma orientação de design.
Ela não descreve roteamento automático de modelos nem um controle de nível de
raciocínio disponível na release atual.

O [checklist de prontidão](release-readiness-1.0.md) é a fonte atual dos gates
abertos. Não trate plano ou nota histórica como evidência de que uma
funcionalidade existe no build atual.

As [notas da versão 1.0.0](releases/v1.0.0.pt-BR.md) e o
[checklist do candidato](release-rc-checklist.pt-BR.md) descrevem o candidato
sem afirmar que a tag estável foi publicada.
