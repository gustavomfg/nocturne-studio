...
import { parseTurnCompletion } from './turnCompletion'
...
export function useTurnLifecycle({ flushStream, activeTurnRef, refreshGit }: { flushStream(): void; activeTurnRef: MutableRefObject<ActiveTurnContext | null>; refreshGit(conversationId: string): Promise<void> }) {
  ...
  return async (params: Record<string, unknown>) => {
    ...
    const completionKey = completion.id ?? `${String(params.threadId ?? 'thread')}:${state.streaming.length}`
    ...
    try {
      ...
      const persistedMessage = persistedAssistantMessage(completion.persistedMessage, context.conversationId)
      let persistenceRejected = false
      if (completion.hasError) store.setError(completion.errorMessage ?? t('common.runNotCompleted'))
      if (completion.persistenceWarning) store.setError(completion.persistenceWarning)
      store.upsertActivity({ id: `completion-${completion.id ?? Date.now()}`, type: 'completion', label: completion.hasError ? t('agent.executionErrorEnded') : cancelled ? t('agent.executionCancelled') : t('agent.executionCompleted'), status: completion.hasError ? 'failed' : 'completed' })
      if (persistedMessage) {
      ...
      if (context.suggestionId && !persistenceRejected) {
        ...
        if (!completion.hasError && !cancelled && changedInApprovedScope) await window.nocturne.suggestions.status(context.conversationId, context.suggestionId, 'resolved', t('common.completedWithChanges'))
        if (useAppStore.getState().activeId === context.conversationId) store.setSuggestions((await window.nocturne.suggestions.page(context.conversationId)).items)
      }
