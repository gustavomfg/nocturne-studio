...
export interface ProjectIndexPanelProps {
  ...
  validationHasMore: boolean
  validationPageLoading: boolean
  onLoadMoreValidations(): void
...
export function ProjectIndexPanel({ workspace, status, summary, stack, symbols, query, loading, onQuery, onSearch, onStart, onCancel, onRetry, validationRuns, validationLoading, validationHasMore, validationPageLoading, onLoadMoreValidations, onValidation, onValidationCancel, semanticStatus, semanticSummary, semanticResults, semanticQuery, semanticLoading, onSemanticQuery, onSemanticSearch, onSemanticStart, onSemanticCancel, engineeringReport, activeConversationId, onCreateSuggestion, showTitle = true }: ProjectIndexPanelProps) {
  const { t } = useI18n()
  ...
  return <div className="project-index-panel">
    ...
    <section className="project-index-section"><h3><Play size={14}/>{t('projectIndex.validation')}</h3><div className="project-index-validation-actions">{validationKinds.map((kind) => <button key={kind} type="button" disabled={validationBusy} onClick={() => onValidation(kind)}>{t(`projectIndex.validation.${kind}`)}</button>)}{validationBusy && <button type="button" onClick={onValidationCancel}><Square size={12}/>{t('projectIndex.cancelValidation')}</button>}</div>{validationRuns.length ? <div className="project-index-validation-runs">{validationRuns.map((run) => <article key={run.id}><header><strong>{t(`projectIndex.validationStatus.${run.status}`)}</strong><small>{run.durationMs !== null ? `${run.durationMs} ms` : '—'}</small></header><code>{run.command ? [run.command, ...run.args].join(' ') : t('projectIndex.validationNotConfigured')}</code>{run.error && <p className="project-index-error"><TriangleAlert size={13}/>{run.error}</p>}{run.outputSummary && <details><summary>{t('projectIndex.validationOutput')}</summary><pre>{run.outputSummary}</pre></details>}</article>)}</div> : <p className="project-index-muted">{t('projectIndex.validationHint')}</p>}{validationHasMore && <button type="button" className="project-index-load-more" disabled={validationPageLoading} onClick={onLoadMoreValidations}>{validationPageLoading ? t('projectIndex.validationLoadingMore') : t('projectIndex.validationLoadMore')}</button>}</section>
