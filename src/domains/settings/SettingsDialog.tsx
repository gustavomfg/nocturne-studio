    ...
    <div ref={dialogRef} className="settings-dialog beta-settings" role="dialog" aria-modal="true" aria-labelledby="settings-title" tabIndex={-1} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="settings-layout">
        ...
        <main className="settings-content">
          ...
          {page === 'application' && <SettingsSection title={t('settings.preferences')}><div className="settings-columns"><label>{t('settings.theme')}<select value="dark" disabled><option value="dark">{t('settings.darkTheme')}</option></select></label><label>{t('settings.language')}<select value={form.language === 'en' ? 'en' : 'pt-BR'} onChange={(event) => setForm({ ...form, language: event.target.value as Language })}><option value="pt-BR">Português (Brasil)</option><option value="en">English</option></select><small>{t('settings.languageHint')}</small></label></div><label className="check-label"><input type="checkbox" checked={Boolean(form.diagnosticMode)} onChange={(event) => setForm({ ...form, diagnosticMode: event.target.checked })}/><span><strong>{t('settings.detailedLogs')}</strong><small>{t('settings.detailedLogsHint')}</small></span></label><button className="secondary-setting" onClick={() => requestExit('onboarding')}>{t('settings.reopenOnboarding')}</button></SettingsSection>}
