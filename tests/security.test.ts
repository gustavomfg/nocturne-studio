...
describe('políticas de execução', () => {
  ...
  it('exercita bloqueios de janela e navegação no smoke empacotado', () => {
    ...
    const smokeRunner = fs.readFileSync(path.join(process.cwd(), 'scripts/smoke-package.mjs'), 'utf8')
    ...
    expect(packageSmoke).toContain('contextIsolationEnabled')
  ...
  it('publica uma release estável somente após verificar o inventário cross-platform', () => {
    const workflow = fs.readFileSync(path.join(process.cwd(), '.github/workflows/stable-release.yml'), 'utf8')
    expect(workflow).toContain('pattern: nocturne-stable-*')
    expect(workflow).toContain('npm run verify:release-assets -- release-assets')
    expect(workflow).toContain('scripts/verify-packaged-metadata.mjs')
    ...
    expect(workflow).toContain("if: matrix.platform == 'linux'")
    expect(workflow).toContain("if: matrix.platform != 'linux'")
    ...
    const backfill = fs.readFileSync(path.join(process.cwd(), '.github/workflows/release-backfill.yml'), 'utf8')
    expect(backfill).toContain('ref: ${{ inputs.release_tag }}')
    expect(backfill).toContain('APPLICATION_SHA: ${{ inputs.application_sha }}')
    expect(backfill).toContain('WORKFLOW_SHA: ${{ github.sha }}')
    expect(backfill).toContain('test "$OPERATIONAL_REF" = "v1.0.1-backfill"')
    expect(backfill).toContain('test "$(git rev-parse HEAD)" = "$APPLICATION_SHA"')
    expect(backfill).toContain('gh release upload "$RELEASE_TAG" --repo "$GITHUB_REPOSITORY" release-assets/*')
    expect(backfill).toContain('npm --prefix tooling run verify:release-assets -- --platforms windows,macos ../release-assets')
    expect(backfill).toContain('environment: stable-release')
    expect(backfill).toContain("APPLE_ID: ''")
  })
