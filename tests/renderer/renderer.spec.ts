...
test.describe('renderer do produto', () => {
  ...
  test('permite selecionar idioma e tema e mantém as preferências após reabrir configurações', async ({ page }) => {
    ...
    const dialog = page.getByRole('dialog', { name: 'Configurações' })
    await dialog.getByRole('button', { name: 'Aplicativo' }).click()
    await dialog.getByLabel('Tema').selectOption('light')
    ...
    await expect(englishDialog.getByLabel('Theme')).toHaveValue('light')
    await englishDialog.getByLabel('Theme').selectOption('dark')
...
test('conclui a jornada de primeiro uso e persiste a decisão', async ({ page }) => {
  ...
  const readiness = onboarding.locator('.readiness-summary')
  await expect(readiness.locator('.readiness-summary-title')).toHaveText('Ambiente pronto para trabalhar')
  await expect(readiness.locator('.readiness-summary-description')).toHaveText('Todas as verificações essenciais foram concluídas.')
  await expect(readiness).toHaveCSS('flex-direction', 'column')
